...
- 📚 **历史记录**: 自动保存所有处理过的音频，支持快速切换和查看历史会话
- 🔄 **会话管理**: 每个音频对应独立会话，可随时切换，会话自动恢复
- 🌊 **流式输出**: 实时显示转录、总结和对话结果，提供流畅的用户体验
- 🎨 **现代UI**: 简洁美观的前端界面，支持拖拽上传，右侧历史记录面板
...
5. 转录完成后，在对话区域输入问题
6. 模型会基于转录内容进行智能回答
7. 右侧历史记录面板可查看和切换所有历史音频
8. 点击历史记录即可切换到该会话，查看历史内容并继续对话
...
### v3.2.0 (最新)
- ✨ 新增"上传新音频"按钮，一键回到上传状态
- ✨ 新增删除历史记录功能，悬停显示删除按钮
- ✨ 对话历史永久保存，切换会话时自动恢复
- 💾 历史记录包含完整对话历史，服务器重启不丢失
- 🔧 改进会话切换体验，对话记录完整保留

### v3.1.0
- ✨ 新增历史记录功能，自动保存所有处理过的音频
- ✨ 右侧历史记录面板，支持快速切换会话
- ✨ 每个音频对应独立会话，可随时查看历史转录和总结
- ✨ 会话自动恢复，过期会话可从历史记录恢复
- 💾 持久化存储，服务器重启后历史记录依然保留
- 🎨 优化界面布局，历史面板固定在右侧
